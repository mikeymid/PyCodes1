...
#################################################
yield_fpath3 = '367_locations_USA&CA.csv'
df3 = pd.read_csv(yield_fpath3)    
#df3 = df3[df3.PIPELINE_FUNCTION.str.contains('Breeding')]
#df3 = df3[~df3.BREEDING_PROGRAM.str.contains('TECH DEV')]
#df3 = df3[df3.LOCATION.str.endswith(', CAN')]
 
df3 = df3['LOCATION_ID'] #df3 is the field names' list
...
#df4 gives the list of location names for which coordinates are required
#df2 provides the yield related information for the selected locations
##################################################
...
if __name__ == '__main__':
    ...
    print('got geo data for ' + str(df.LOCATION_ID.nunique()) + ' fields')

    fld_drive_flds = df[df.source == 'FIELD_DRIVE'].LOCATION_ID.unique()

    all_flds = df.LOCATION_ID.unique()

    #print(str(df.LOCATION_ID.nunique()) + ' of those fields were FIELD_DRIVE')
    
   
    flds_missing_fld_drive = []
    for fld in all_flds:
        if fld not in fld_drive_flds:
            flds_missing_fld_drive.append(fld)

    
    
    new_df = pd.DataFrame()
    new_df['flds'] = flds_missing_fld_drive
    new_df.to_csv('fields_missing_field_drive.csv')
    
    
    #flds_missing_fld_drive.to_csv('output3.csv')
    #print(str(flds_missing_fld_drive.LOCATION_ID.nunique()) + ' of those fields were missing in FIELD_DRIVE')
