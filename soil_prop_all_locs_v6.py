...
def make_fig_multi_core(feat='ph', flds=['MNCF','MNWX','MNPX'], 
    # {{{
        min_n_samples_for_kriging=2500, show_fig=False):
    ...
    #cmap = create_stepwise_cmap(feat, colors, positions) 
    ...
    yield_df, soil_df = location_selection.load_data(soil_fpath, yield_fpath,
            max_n_samples_per_field=1000000000)
    ...
    for i, fld in tqdm(enumerate(flds), desc='Fields'):
        ...
        if i == 0:
            ...
            ax = fig.add_axes(rect, sharex=first_ax, sharey=first_ax)
        ...
        #plot_single_field(ax, soil_df, fld, feat, min_n_samples_for_kriging)


    # create parameters for multiprocessing
    inputs = []
    for i in flds:
        inputs.append((axes_by_fld[fld], soil_df, fld, feat, 
                min_n_samples_for_kriging))
    
    with Pool() as pool:
        pool.starmap(plot_single_field, inputs)
...
def plot_single_field(ax, soil_df, fld, feat, min_n_samples_for_kriging):
        # {{{
        ...
        if do_kriging:
            ...
            c = np.vstack(soil_df.color.values)
            ax.scatter(x, y, c=c, edgecolor='none', s=40, alpha=0.5)
            #circle_scatter(ax, x, y, c, radius=1)

            ax.set_xlim(fld_df.x.min(), fld_df.x.max())
            ax.set_ylim(fld_df.y.min(), fld_df.y.max())
            ax.set_aspect('equal')
            ax.text(0.02, 0.98, (fld), verticalalignment='top', alpha=0.5,
...
def make_fig(soil_df, feat='ph', flds=['MNCF','MNWX','MNPX'], 
    ...
    feat_colors = {
            ...
            'organicMatter':{'break_points':[3, 4],
                'colors':[
                    red,
                    green,
                    dark_blue
    ...
    for i, fld in tqdm(enumerate(flds), desc='Fields'):
        ...
        if do_kriging:
            ...
            if True:#num_samples < 500000:
                s = (800/float(np.sqrt(num_samples)) * 10 * 
...
if __name__ == '__main__':
    ...
    if True:
        make_fig(soil_df, flds=flds, feat=feat, show_fig=True, nlags=nlags, 
                max_samples=max_samples,
